    ...
    $(function(){
        $('#city').on('focus',function(){
            ...
            google.maps.event.addListener(city, 'place_changed', function() {
                place = city.getPlace();
                $('#city').text(place.address_components[0].long_name);
            })
        })
    });
</script>
<form action="<?php echo $_SERVER['PHP_SELF']?>" method="POST">
    ...
    <table cellpadding=4 cellspacing=1 border="0">
        <tr valign="top">
            <td class="formheading" colspan="2"><?=$lang->getLang('Add a Region')?></td>
        ...
        <tr valign="top">
            ...
            <td class="content"><input type=text id="region_name" class="lettersAndNumbers" name="region_name" size="26" value="<?=replace_quotes($_POST['region_name']);?>"></td>
		</tr>
            <?php
		   ...
		   <td class=content>
               <select name="nation_id" id="nation_id" onchange="getSelectedValue();">
                   ...
                   if($_SESSION["jurisdiction_class"] == "nation"){
                       ...
                       echo  get_jurisdiction_options('nation', $_POST['nation_id'], $hide_internet=true);
                   }
                   ?>
               </select>
		   </td>
        ...
        <tr>
            ...
            <td class=content><?$jurSkins=getJurisdictionSkins($skinId);
                ?>
                <select name="skin">
                    <?
                    while($jurSkins->hasNext()){
                        $row=$jurSkins->next();
                        ?>
                        <option value='<?=$row['skn_id']+53?>'> <?=$row['skn_name']?></option>
                    <?
                    }
                      ?>
                </select>
        ...
        <table id="casino_perc" width="90%">
            <?php
                ...
                showPercentage($_SESSION["jurisdiction_id"],$_SESSION["jurisdiction_class"],-1,"region");
            ?>
        </table>
        ...
        <table id="betting_perc" width="90%">
            <?php
                showBettingPercentage($_SESSION["jurisdiction_id"],$_SESSION["jurisdiction_class"], "region",-1,-1);
            ?>
        </table>
        ...
        <table id="betting_live_perc" width="90%">
            <?php
            showLiveBettingPercentage($_SESSION["jurisdiction_id"],$_SESSION["jurisdiction_class"], "region",-1, -1);
            ?>
        </table>
        ...
        <table cellpadding=0 cellspacing="1" border=0 >
            <tr>
                <td colspan='2'>
                    <a href="javascript:void(0);" onclick="showAdminAdd();" id="adminAddShow"><?=$lang->getLang("Add an administrator for this %" , "region")?> &#9660;</a>
                </td>
            </tr>
        </table>
                        ...
                        <tr valign="top">
                            ...
                            <td <? form_td_style('user_type'); ?> >
                                <select name='user_type'>
                                    <option value="4"><?=$lang->getLang("Region Manager")?></option>
                                </select>
...
<script type="text/javascript">
    ...
    function showAdminAdd(){
        if ($("#adminAdd").is(":hidden")){
            ...
            $("#adminAddShow").html("<?=$lang->getLang("Add an administrator for this %" , "region")?> &#9660;");
        }
        return false;
    }
