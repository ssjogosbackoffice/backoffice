<script type="text/javascript" src="https://maps.googleapis.com/maps/api/js?libraries=places&sensor=false&&language=en&region=US">
</script>
<script language="javascript" type="text/javascript">
    ...
    function doRefresh(form) {
        var msg = "<?=$lang->getLang("Refreshing will reload the saved description for this %, causing any unsaved changes to be lost. Are you sure you want to do this?","region")?>";
    ...
    function deleteJurisdiction(jur_id){
        jConfirm("<?=$lang->getLang("Block %?",$region_name)?>","",function (answer) {
            if(answer){
                var lock_users = false;
                var lock_punters = false;
                jConfirm("<?=$lang->getLang("Lock all % of % structure?",$lang->getLang("administrators"),$region_name)?>","",function (answer) {
                    lock_users = answer;
                    jConfirm("<?=$lang->getLang("Lock all % of % structure?",$lang->getLang("players"),$region_name)?>","",function (answer) {
                        lock_punters = answer;
    ...
    function restoreJurisdiction(jur_id){
        jConfirm("<?=$lang->getLang("Unblock %?",$region_name)?>","",function (answer) {

            if(answer){
                var unlock_users = false;
                var unlock_punters = false;
                jConfirm("<?=$lang->getLang("Unlock all % of % structure?",$lang->getLang("administrators"),$region_name)?>","",function (answer) {
                    unlock_users = answer;
                    jConfirm("<?=$lang->getLang("Unlock all % of % structure?",$lang->getLang("players"),$region_name)?>","",function (answer) {
...
if($canModify) { ?>
        <form name="region_display_form" action="<?=$_SERVER['PHP_SELF']?>" method="POST">
            <input type="hidden" name="node" value="<?=$node?>" />
            <input type="hidden" name="class" value="region" />
                    ...
                    <tr valign="top">
                        <td>
                            <table cellpadding= "4" cellspacing=1 border=0>
                                <tr valign="top">
                                    <td class="formheading" colspan="2"><?=$lang->getLang("Details of")?> <?=$region_name?></td>
                                </tr>
                                <tr valign="top">
                                    <td class=label><?=$lang->getLang("Region name")?></td>
                                    <td class=content><input type="text" class="lettersAndNumbers" name="region_name" id="region_name" value="<?=$region_name;?>" /></td>
                                    <td></td>
                                </tr>
                                <tr>
                                    <td class=label width=110><?=$lang->getLang("Account Code")?> <br /><div class="labelnote">(6 Characters)</div></td>
                                ...
                                <tr valign="top">
                                    <td class=label><?=$lang->getLang("Nation")?> </td>
                                    <td class=content>
                                        <select name="nation_id" id="nation_id" onchange="getSelectedValue();">
                                            <?php
                                            if($_SESSION["jurisdiction_class"] == "nation"){
                                                echo  get_jurisdiction_options('nation', $id=$_SESSION['jurisdiction_id'], $hide_internet=true);
                                            }else{
                                                echo  get_jurisdiction_options('nation', $parent_id, $hide_internet=true);
                                            }
                                            ?>
                                        </select>
                                ...
                                <? if (check_access("delete_jurisdictions", false)) :?>
                                    <? if($jur_status == 1) : ?>
                                        <tr valign="top">
                                            ...
                                            <td class="content"><button onclick="return deleteJurisdiction(<?=$node?>);"><?=$lang->getLang("Block %?",$region_name)?></button></td>
                                        </tr>
                                    <?php endif; ?>
                                    <? if($jur_status == 0) : ?>
                                        <tr valign="top">
                                            <td class="label" width="130"><?=$lang->getLang("Access")?></td>
                                            <td class="content"><button onclick="return restoreJurisdiction(<?=$node?>);"><?=$lang->getLang("Unblock %?",$region_name)?></button></td>
                                        </tr>
                                    <?php endif; ?>
                                <?php endif; ?>
                                ...
                                <tr valign="top">
                                    ...
                                    <td class="content">
                                        ...
                                        <br>
                                        <button id="update_processor" data-jur="<?=$node?>" data-name="<?=$region_name?>" data-class="region" class="btn btn-small"><?=$lang->getLang("Update sub")?></button>
                                    </td>
                                </tr>
                                ...
                                <table id="casino_perc" width="90%">
                                    <?php
                                        ...
                                        showPercentage($_SESSION["jurisdiction_id"],$_SESSION["jurisdiction_class"],$node, "region");
                                    ?>
                                </table>
                                <table id="error_mess_payments" width="90%" style="display:none;">
                                    ...
                                    <tr>
                                        <td class='content' colspan='2'><?=$lang->getLang("Before to insert percentage to this new % your up level has to set your percentage!","region")?></td>
                                    </tr>
                                </table>
                                <table id="betting_perc" width="90%">
                                    <?php
                                        showBettingPercentage($_SESSION["jurisdiction_id"],$_SESSION["jurisdiction_class"], "region",$node, check_access("betting_percentage_setting"));
                                    ?>
                                </table>
                                ...
                                <table id="betting_live_perc" width="90%">
                                    <?php
                                    showLiveBettingPercentage($_SESSION["jurisdiction_id"],$_SESSION["jurisdiction_class"], "region",$node, check_access("betting_percentage_setting"));
                                    ?>
                                </table>
                        ...
                        <td width=80>
                            ...
                            <? if($canModify) { ?>
                            <input type="submit" class="entitybtn disabledBtn" id="sub_update" style="width:75px;margin-bottom:2px;" value="<?=$lang->getLang("Update")?>" />
                            <br />
