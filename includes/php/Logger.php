...
/**
 * This is the central class in the logger package. All logging operations 
...
class Logger {
	...
	/** The logger hierarchy used by logger. */
	private static $hierarchy;
	
	/** Inidicates if logger has been initialized */
	...
	private static function getConfigurator($configurator = null) {
		...
		if (is_object($configurator)) {
			if ($configurator instanceof LoggerConfigurator) {
				...
				trigger_error("logger: Given configurator object [$configurator] does not implement the LoggerConfigurator interface. Reverting to default configurator.", E_USER_WARNING);
				return new LoggerConfiguratorDefault();
			}
		}
		...
		if (is_string($configurator)) {
			if (!class_exists($configurator)) {
				trigger_error("logger: Specified configurator class [$configurator] does not exist. Reverting to default configurator.", E_USER_WARNING);
				return new LoggerConfiguratorDefault();
			}
			...
			if (!($instance instanceof LoggerConfigurator)) {
				trigger_error("logger: Specified configurator class [$configurator] does not implement the LoggerConfigurator interface. Reverting to default configurator.", E_USER_WARNING);
				return new LoggerConfiguratorDefault();
			}
		...
		trigger_error("logger: Invalid configurator specified. Expected either a string or a LoggerConfigurator instance. Reverting to default configurator.", E_USER_WARNING);
		return new LoggerConfiguratorDefault();
	}
	
	/**
	 * Returns true if the logger framework has been initialized.
	 * @return boolean 
	 */
