 ...
 * 
 * The pool is used when configuring logger. First all appender instances 
...
class LoggerAppenderPool {
	...
	public static function add(LoggerAppender $appender)
	{
		...
		if(empty($name)) {
			trigger_error('logger: Cannot add unnamed appender to pool.', E_USER_WARNING);
			return;
		}
		
		if (isset(self::$appenders[$name])) {
			trigger_error("logger: Appender [$name] already exists in pool. Overwriting existing appender.", E_USER_WARNING);
		}
