		...
		foreach($options as $option) {
			if (!empty($option)) {
				...
				if (defined($constant)) {
					...
					trigger_error("logger: Invalid syslog option provided: $option. Whole option string: {$this->option}.", E_USER_WARNING);
				}
			}
		}
		return $value;
	...
	private function parseFacility() {
		if (!empty($this->facility)) {   
			...
			if (defined($constant)) {
				...
				trigger_error("logger: Invalid syslog facility provided: {$this->facility}.", E_USER_WARNING);
			}
		}
	}
	...
	private function parsePriority() {
		if (!empty($this->priority)) {
			...
			if (defined($constant)) {
				...
				trigger_error("logger: Invalid syslog priority provided: {$this->priority}.", E_USER_WARNING);
			}
		}	
	}
}
